...
export const getHfDatasetInfo = async (token: string, hfPath: string) => {
	let error = null;

	const res = await fetch(
		`${WEBUI_API_BASE_URL}/knowledge/hf/dataset-info?hf_path=${encodeURIComponent(hfPath)}`,
		{
			method: 'GET',
			headers: {
				Accept: 'application/json',
...
export const getHfDatasetRows = async (token: string, hfPath: string, length: number = 5) => {
	let error = null;

	const res = await fetch(
		`${WEBUI_API_BASE_URL}/knowledge/hf/dataset-rows?hf_path=${encodeURIComponent(hfPath)}&length=${length}`,
		{
			method: 'GET',
			headers: {
				Accept: 'application/json',
...
// Row preview for a local (curated) dataset — samples the KB's own JSONL files.
// Same {columns, rows} shape as getHfDatasetRows, used when there's no hf_path.
export const getDatasetRows = async (token: string, id: string, length: number = 5) => {
	let error = null;

	const res = await fetch(
		`${WEBUI_API_BASE_URL}/knowledge/${id}/dataset-rows?length=${length}`,
		{
			method: 'GET',
			headers: {
				Accept: 'application/json',
