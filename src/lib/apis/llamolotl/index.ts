...
export const getAvailableLlamolotlModels = async (token: string, urlIdx: number | null = null) => {
    let error = null;
...
/** A model's slot occupancy. `null` means the router would not say — which is
 *  NOT the same as zero, and the swap loop must never treat it as a green light. */
export type LlamolotlSlots = { busy: number; total: number } | null;

export type LlamolotlResidencyModel = {
    id: string;
    status?: string;
    vram_footprint_bytes?: number;
    vram_footprint_source?: string;
    slots?: LlamolotlSlots;
};

export type LlamolotlResidency = {
    models: LlamolotlResidencyModel[];
    capacity: {
        total_bytes: number;
        free_bytes: number;
        /** What llamolotl can free by evicting its own residents. A model too big
         *  for free_bytes alone is still admissible against free + this (self.ai#114). */
        llamolotl_held_bytes: number;
    };
};

/** Residency, cost and in-flight generation in ONE read. Deliberately not three
 *  calls: the model list and the slot reads would describe different moments,
 *  and the swap gate would fire on a snapshot that never existed. */
export const getLlamolotlResidency = async (token: string): Promise<LlamolotlResidency> => {
    let error = null;

    const res = await fetch(`${LLAMOLOTL_API_BASE_URL}/residency`, {
        method: 'GET',
        headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
...
export const unloadAllLlamolotlModels = async (token: string, urlIdx: number | null = null) => {
    let error = null;
