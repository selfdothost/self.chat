<script>
	...
	// Pipelines.svelte (the inherited OpenWebUI Pipelines plugin framework) is
	// deliberately unwired from this nav — see self.chat#30. It's unrelated to
	// self.curator's own pipeline editor (Knowledge base > Pipeline tab) and,
	// with no OWUI pipelines server ever deployed here, only ever showed
	// "Pipelines Not Detected". The component file stays in the tree in case a
	// future OWUI-compatible Functions-as-pipelines use case revives it.
...
<div class="flex flex-col lg:flex-row w-full h-full pb-2 lg:space-x-4">
	...
	>
		...
		>
			...
			<div class=" self-center">{$i18n.t('Backup')}</div>
		</button>
	</div>
	...
	<div class="flex-1 mt-3 lg:mt-0 overflow-y-scroll pr-1 scrollbar-hidden">
		...
		{:else if selectedTab === 'db'}
			<!-- Read-only: nothing on this page is settable, so no saveHandler. -->
			<Database />
