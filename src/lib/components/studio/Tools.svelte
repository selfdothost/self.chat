<script lang="ts">
	...
	const cloneHandler = async (tool) => {
		...
		if (_tool) {
			...
			goto(resolve('/(app)/studio/tools/create'));
		}
	};
		...
		<div class=" flex w-full space-x-2">
			...
			<div>
				<a
					...
					href={resolve('/(app)/studio/tools/create')}
				>
		...
		{#each filteredItems as tool (tool.id)}
			...
			>
				<a
					...
					href={resolve(`/(app)/studio/tools/edit?id=${encodeURIComponent(tool.id)}`)}
				>
				...
				<div class="flex flex-row gap-0.5 self-center">
					...
					{:else}
						...
						<ToolMenu
							editHandler={() => {
								goto(resolve(`/(app)/studio/tools/edit?id=${encodeURIComponent(tool.id)}`));
							}}
							cloneHandler={() => {
								cloneHandler(tool);
							}}
