<script lang="ts">
	...
	// PipelineCanvas seeds its internal xyflow state ONCE, from `nodes`/`connections`
	// at mount (an `untrack` in the component, deliberately -- see below). So handing
	// it a freshly loaded graph is not enough; it has to be re-created. Bumping this
	// token remounts it via the `{#key}` at the call site.
	//
	// This is a token rather than an `$effect` in the canvas, and it is keyed on the
	// token rather than on `pipelineNodes`, for the same reason: the canvas emits
	// `onConfigChange`, which is written straight back into `pipelineNodes` below.
	// An effect re-seeding from the prop would read what it transitively writes, and
	// keying on the array would tear the canvas down on every edit -- both reintroduce
	// the self-retriggering render the xyflow rewrite existed to remove (self.chat#28).
	// Only the load path writes this; the canvas never can.
	let pipelineLoadToken = $state(0);
	...
	const loadPipelineHandler = async (fileId) => {
		try {
			...
			if (loadedFile) {
				...
				// Remount the canvas so it re-seeds from the graph we just loaded.
				pipelineLoadToken += 1;
			} else {
...
<div class="flex flex-col w-full translate-y-1" id="collection-container">
	{#if id && knowledge}
		...
		<div style="height: calc(100vh - 270px);">
			{#key pipelineLoadToken}
				<PipelineCanvas nodes={pipelineNodes} connections={pipelineConnections} onConfigChange={(detail) => { pipelineNodes = detail.nodes; pipelineConnections = detail.connections; }}/>
			{/key}
		</div>
