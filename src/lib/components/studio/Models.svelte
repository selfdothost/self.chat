<script lang="ts">
	...
	const deleteModelHandler = async (model) => {
		...
		models = await getStudioModels(localStorage.token);
	};
	...
	const cloneModelHandler = async (model) => {
		...
		goto(resolve('/(app)/studio/models/create'));
	};
		...
		<div class=" flex flex-1 items-center w-full space-x-2">
			...
			<div>
				<a
					...
					href={resolve('/(app)/studio/models/create')}
				>
						...
						{:else}
							{#if $user?.role === 'admin' || model.user_id === $user?.id}
								<a
									class="self-center w-fit text-sm px-2 py-2 dark:text-gray-300 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5 rounded-xl"
									type="button"
									href={resolve(`/(app)/studio/models/edit?id=${encodeURIComponent(model.id)}`)}
								>
									<svg
