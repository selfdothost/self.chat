	...
	>
		<div class="my-2">
			<div class="flex flex-col w-full">
				...
				<!--
					Scoped to the Command row ONLY. This Tooltip used to open above
					the whole flex-col -- Title input, Access button and Command row
					together -- while its content describes just the Command field.
					Tooltip passes no `trigger` to tippy, so tippy's default
					'mouseenter focus' applies and ANY focus or hover inside that
					region opened it; with placement="bottom-start" it rendered over
					the "Prompt Content" heading below (self.chat#23).

					Forcing trigger: 'mouseenter' would also suppress it, but drops
					the keyboard and screen-reader route to the helper text.
					Narrowing the wrapper keeps that path and fixes the cause.

					className="w-full" preserves the previous layout: Tooltip renders
					a plain div carrying className, whose default value is "flex" --
					which would make this row a flex item that no longer spans the
					full width.
				-->
				<Tooltip
