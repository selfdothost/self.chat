<script lang="ts">
	...
	import {
		...
		type EvalTask,
		type EvalType
	} from '$lib/apis/evaluations/tasks';
	...
	// $state because the availability check in the modal reads it (self.chat#40).
	// It was mutated without being reactive before; nothing depended on it, so
	// the bug was latent.
	let activeEvalType: 'code-eval' | 'language-eval' = $state('code-eval');
	...
	// ── Live catalog (self.chat#40) ─────────────────────────────────────
	//
	// The curated arrays above stay: they carry descriptions, problem counts and
	// difficulty that the harness does not report, and a flat 14,115-row list is
	// not a usable picker. What they must NOT do any more is decide what EXISTS.
	// They drifted to 144 hand-maintained entries against a live catalog that is
	// two orders of magnitude larger, and — the reason this became urgent — an
	// evaluation registered at runtime (self.ai#91) could never appear in them.
	//
	// So: curation decides presentation, the harness decides availability.

	type CatalogState = 'unknown' | 'loaded' | 'error';

	let liveTaskIds: Record<EvalType, Set<string>> = $state({
		'code-eval': new Set(),
		'language-eval': new Set()
	});
	let catalogState: Record<EvalType, CatalogState> = $state({
		'code-eval': 'unknown',
		'language-eval': 'unknown'
	});
	let catalogError = $state('');
	let liveLanguages: { builtin: string[]; custom: string[] } = $state({ builtin: [], custom: [] });
	let languagesState: CatalogState = $state('unknown');

	/**
	 * Is this curated task actually runnable right now?
	 *
	 * Returns true when the catalog has not loaded or failed to load. That is
	 * deliberate: greying every entry out because the harness is unreachable
	 * would present an outage as "these benchmarks do not exist", which is the
	 * exact conflation the API is careful to avoid (a 502 is not an empty list).
	 * An unverified entry is shown as normal, and the banner says availability
	 * could not be checked.
	 */
	const isAvailable = (evalType: EvalType, taskId: string): boolean => {
		if (catalogState[evalType] !== 'loaded') return true;
		return liveTaskIds[evalType].has(taskId);
	};

	const loadCatalog = async (evalType: EvalType) => {
		try {
			const tasks: EvalTask[] = await getEvalTasks(getToken(), evalType);
			liveTaskIds[evalType] = new Set(tasks.map((t) => t.name));
			catalogState[evalType] = 'loaded';
		} catch (e) {
			// Keep "could not check" apart from "found nothing".
			catalogState[evalType] = 'error';
			catalogError = typeof e === 'string' ? e : $i18n.t('Could not verify which benchmarks are available.');
		}
	};

	const loadLanguages = async () => {
		try {
			liveLanguages = await getEvalLanguages(getToken());
			languagesState = 'loaded';
		} catch (e) {
			languagesState = 'error';
			console.error('Failed to list MultiPL-E languages:', e);
		}
	};

	/**
	 * Human labels for the MultiPL-E tokens the harness reports.
	 *
	 * A lookup, not a gate: a token with no entry here still renders, under its
	 * own name. That is the whole point — a language registered at runtime is
	 * unknown to this map by definition, and dropping it would reintroduce the
	 * bug one level down.
	 */
	const MULTIPLE_LANGUAGE_NAMES: Record<string, string> = {
		js: 'JavaScript', ts: 'TypeScript', java: 'Java', cpp: 'C++', cs: 'C#',
		go: 'Go', rs: 'Rust', rb: 'Ruby', php: 'PHP', sh: 'Bash', scala: 'Scala',
		lua: 'Lua', r: 'R', pl: 'Perl', rkt: 'Racket', d: 'D', jl: 'Julia',
		swift: 'Swift', ml: 'OCaml', hs: 'Haskell', elixir: 'Elixir',
		clj: 'Clojure', dart: 'Dart', py: 'Python'
	};

	/**
	 * The curated code-eval groups, with MultiPL-E rebuilt from the harness.
	 *
	 * MultiPL-E is the one curated group whose membership is genuinely dynamic:
	 * languages are registrable at runtime, and the hardcoded 15 sat against 23
	 * built-ins. Every other code-eval group is a fixed dataset, so its curated
	 * task list stays and is merely availability-checked.
	 */
	let codeGroups = $derived.by(() => {
		if (languagesState !== 'loaded') return benchmarkGroups;
		const tokens = [...liveLanguages.builtin, ...liveLanguages.custom];
		return benchmarkGroups.map((g) =>
			g.id !== 'multiple'
				? g
				: {
						...g,
						tasks: tokens.map((tok) => ({
							id: `multiple-${tok}`,
							name: MULTIPLE_LANGUAGE_NAMES[tok] ?? tok,
							count: 164,
							difficulty: 'Medium'
						}))
					}
	...
	/**
	 * Live tasks that no curated group mentions — pinned to the top.
	 *
	 * This is what makes a just-added evaluation findable without knowing its
	 * name, which was the concrete complaint: registering, syncing and
	 * validating an eval left it invisible everywhere in this UI.
	 *
	 * Derived by subtraction rather than by asking which evals are custom. The
	 * two questions differ — a task can be uncurated without being
	 * admin-registered — and subtraction is the one that answers "what would I
	 * otherwise never see here".
	 */
	const uncuratedGroupFor = (evalType: EvalType, curated: { tasks: { id: string }[] }[]) => {
		if (catalogState[evalType] !== 'loaded') return null;
		const curatedIds = new Set(curated.flatMap((g) => g.tasks.map((t) => t.id)));
		const extras = [...liveTaskIds[evalType]].filter((id) => !curatedIds.has(id)).sort();
		if (extras.length === 0) return null;
		return {
			// `gated` is part of the shape the curated language-eval groups use
			// (HF-auth-required datasets). Always false here — the harness has
			// already reported these as runnable.
			gated: false,
			id: `__uncurated_${evalType}`,
			name: $i18n.t('Other available benchmarks'),
			description: $i18n.t(
				'Reported by the harness but not part of a curated group — including evaluations added at runtime.'
			),
			language: '—',
			tasks: extras.map((id) => ({ id, name: id, count: 0, difficulty: 'Medium' }))
		};
	};

	let uncuratedCode = $derived(uncuratedGroupFor('code-eval', codeGroups));
	let uncuratedLanguage = $derived(uncuratedGroupFor('language-eval', leaderboardGroups));

	// A curated group is tens of entries; the uncurated language-eval group is
	// ~14,000. Search appears only when the list is big enough to need it, and
	// rendering is capped so a large group cannot lock the modal up.
	const TASK_SEARCH_THRESHOLD = 40;
	const MAX_RENDERED_TASKS = 200;

	let taskSearch = $state('');

	type GroupTask = { id: string; name: string; count: number; difficulty: string; category?: string };

	const matchingTasks = (tasks: GroupTask[]): GroupTask[] => {
		const q = taskSearch.trim().toLowerCase();
		if (!q) return tasks;
		return tasks.filter((t) => t.id.toLowerCase().includes(q) || t.name.toLowerCase().includes(q));
	};

	/**
	 * What actually renders: filtered, then capped.
	 *
	 * Anything already selected is kept regardless of the cap — otherwise a
	 * selection could sit outside the rendered window and be silently submitted
	 * with no way to see or clear it.
	 */
	const visibleTasks = (tasks: GroupTask[]): GroupTask[] => {
		const matched = matchingTasks(tasks);
		if (matched.length <= MAX_RENDERED_TASKS) return matched;
		const head = matched.slice(0, MAX_RENDERED_TASKS);
		const shown = new Set(head.map((t) => t.id));
		const strays = matched.filter((t) => selectedTasks.has(t.id) && !shown.has(t.id));
		return [...strays, ...head];
	};
	...
	const openGroupModal = async (group: BenchmarkGroup, evalType: 'code-eval' | 'language-eval' = 'code-eval') => {
		...
		taskSearch = '';
		// Don't preselect a group that is thousands of entries long — "run
		// everything" is a reasonable default for a curated dataset and a
		// terrible one for the uncurated bucket.
		selectedTasks =
			group.tasks.length > TASK_SEARCH_THRESHOLD ? new Set() : new Set(group.tasks.map((t) => t.id));
		jobModelId = '';
		showJobModal = true;
	...
	onMount(async () => {
		// Availability is fetched alongside the jobs rather than lazily on modal
		// open: the tiles themselves need it, to grey out a curated benchmark the
		// harness does not report and to surface the uncurated group at all.
		void Promise.all([loadCatalog('code-eval'), loadCatalog('language-eval'), loadLanguages()]);
		try {
			jobs = await getEvalJobs(getToken());
		} catch (e) {
...
{:else}
	...
	<!--
		Availability could not be checked. Stated rather than shown as an empty
		or all-greyed catalog: a 502 from a harness is not the same answer as
		"this harness has no benchmarks", and rendering them alike is how an
		outage gets mistaken for a configuration problem.
	-->
	{#if catalogState['code-eval'] === 'error' || catalogState['language-eval'] === 'error'}
		<div class="mt-3 px-3 py-2 rounded-xl border border-yellow-300 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-950/30 text-xs text-yellow-800 dark:text-yellow-300">
			{$i18n.t('Could not verify which benchmarks the harness can run, so none are greyed out. Runs may still fail.')}
			{#if catalogError}<span class="opacity-75"> — {catalogError}</span>{/if}
		</div>
	{/if}

	<!-- Language & Understanding Benchmark Cards -->
	<div class="mt-3 mb-5 grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3">
		{#each [...leaderboardGroups, ...(uncuratedLanguage ? [uncuratedLanguage] : [])] as group (group.id)}
			<button
	...
	<div class="mt-3 mb-5 grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3">
		{#each [...codeGroups, ...(uncuratedCode ? [uncuratedCode] : [])] as group (group.id)}
			<button
...
{#if showJobModal && activeGroup}
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onclick={self(() => (showJobModal = false))}>
		<div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-6 w-full mx-4 max-h-[90vh] flex flex-col {activeGroup.tasks.length > 20 ? 'max-w-2xl' : 'max-w-md'}">
			...
			{:else}
				{@const shownTasks = visibleTasks(activeGroup.tasks)}
				{@const hiddenCount = matchingTasks(activeGroup.tasks).length - shownTasks.length}
				{@const hasCategories = shownTasks.some((t) => t.category)}
				{@const categories = hasCategories ? [...new Set(shownTasks.map((t) => t.category))] : [null]}
				<div class="mb-4 flex-1 overflow-hidden flex flex-col min-h-0">
					<div class="flex items-center justify-between mb-1.5">
						...
						<div class="flex gap-2">
							<button
								...
								onclick={() => { selectedTasks = new Set(shownTasks.map((t) => t.id)); }}
					...
					{#if activeGroup.tasks.length > TASK_SEARCH_THRESHOLD}
						<input
							type="text"
							bind:value={taskSearch}
							placeholder={$i18n.t('Search {{count}} tests…', { count: activeGroup.tasks.length })}
							class="w-full mb-1.5 px-3 py-1.5 text-sm rounded-lg border border-gray-200 dark:border-gray-700 bg-transparent"
						/>
					{/if}
					<div class="overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-xl divide-y divide-gray-100 dark:divide-gray-800">
						{#each categories as cat (cat)}
							{@const catTasks = cat ? shownTasks.filter((t) => t.category === cat) : shownTasks}
							{#if cat}
							...
							{#each catTasks as task (task.id)}
								{@const available = isAvailable(activeEvalType, task.id)}
								<label
									class="flex items-center gap-3 px-3 py-2 transition {available
										? 'hover:bg-gray-50 dark:hover:bg-gray-800/50 cursor-pointer'
										: 'opacity-50 cursor-not-allowed'}"
									title={available ? '' : $i18n.t('Not reported by the harness — it cannot run this benchmark.')}
								>
									<input
										type="checkbox"
						...
						{#if hiddenCount > 0}
							<div class="px-3 py-2 text-[11px] text-gray-500 dark:text-gray-400">
								{$i18n.t('{{count}} more not shown — refine your search.', { count: hiddenCount })}
							</div>
						{/if}
					</div>
