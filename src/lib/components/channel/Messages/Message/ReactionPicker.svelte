<script lang="ts">
	...
	// derives to a filtered subset (search results), which is still a
	// string/string[]-keyed map but not the same literal type.
	let emojis: Record<string, string | string[]> = $derived(
		search
			? Object.keys(emojiShortCodes).reduce((acc, key) => {
