<script>
	...
	import { createNewVoice, getVoices } from '$lib/apis/voices';
	...
	const submitHandler = async () => {
		...
		const res = await createNewVoice(localStorage.token, {
			name,
			description
		}).catch((e) => {
			toast.error(e);
		...
		if (res) {
			toast.success($i18n.t('Voice created successfully.'));
			voices.set(await getVoices(localStorage.token));
			goto(resolve('/(app)/studio/voices/[id]', { id: res.id }));
		}
...
<div class="w-full max-h-full">
	<button
		...
		onclick={() => {
			goto(resolve('/(app)/studio/voices'));
		}}
	>
	...
	>
		<div class=" w-full flex flex-col justify-center">
			<div class=" text-2xl font-medium font-primary mb-2.5">
				{$i18n.t('Create a voice')}
			</div>

			<div class="w-full flex flex-col gap-2.5">
				<div class="w-full">
					<div class=" text-sm mb-2">{$i18n.t('What is this voice called?')}</div>
					...
					<div class="w-full mt-1">
						<input
							...
							type="text"
							bind:value={name}
							placeholder={$i18n.t('Name your voice')}
							required
						/>
					</div>
				</div>
				...
				<div>
					...
					<div class=" w-full mt-1">
						<textarea
							...
							placeholder={$i18n.t('Describe this voice (optional)')}
						></textarea>
					</div>
				</div>
			</div>
		...
		<div class="flex justify-end mt-2">
			<div>
				<button
					...
					disabled={loading}
				>
					<div class=" self-center font-medium">{$i18n.t('Create Voice')}</div>
