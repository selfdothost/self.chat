<script lang="ts">
	...
	import PromptEditor from '$lib/components/studio/Prompts/PromptEditor.svelte';
	...
	const onSubmit = async (_prompt) => {
		...
		if (prompt) {
			...
			await goto(resolve('/(app)/studio/prompts'));
		}
	};
	...
	onMount(async () => {
		window.addEventListener('message', async (event) => {
			if (
				!['https://selfai.com', 'https://www.selfai.com', 'http://localhost:5173'].includes(
					event.origin
				)
			)
				return;
